...
<main id="app">
    ...
    <section class="wrapper ai-window">
        ...
        <!-- Chat window -->
        <table v-for="a in answers" class="chat-window" v-bind:key="a.id">
            ...
            <tr>
                <td>
                    ...
                    <div v-for="r in a.result.fulfillment.messages" v-bind:key="r.id">
                        ...
                        <div class="mdc-card" v-if="r.type == 'basic_card'">
                            ...
                            <section class="mdc-card__actions" v-for="button in r.buttons"  v-bind:key="button.id">
                        ...
                        <!-- Bot messazge types / List -->

                        <div class="mdc-list-group mdc-card" v-if="r.type == 'list_card'">
                            <h3 class="mdc-list-group__subheader">{{r.title}}</h3>
                            <ul class="mdc-list mdc-list--two-line mdc-list--avatar-list" v-for="item in r.items" @click="autosubmit(item.optionInfo.key)" v-bind:key="item.id">
                        ...
                        <div v-if="r.type == 'suggestion_chips'" class="chips">
                            <div class="suggestion" @click="autosubmit(s.title)" v-for="s in r.suggestions" v-bind:key="s.id">
                                {{s.title}} ppppp
                            </div>
                        </div>
                        ...
                        <div v-if="r.unknown == true" class="google-chip chips">
                            <a class="suggestion" :href="'https://www.google.com/search?q=' + r.text" target="_blank">
                                Busca para"{{r.text}}" en Google <i class="material-icons openlink">search</i>
                            </a>
                        </div>
        ...
        <div id="start" v-if="showStart">
            <h6 style="margin:0">Califícanos: </h6>
            <div class="ec-stars-wrapper">
                <a href="#" data-value="1" @click="getValor('1')" title="Votar con 1 estrellas">&#9733;</a>
                <a href="#" data-value="2" @click="getValor('2')" title="Votar con 2 estrellas">&#9733;</a>
                <a href="#" data-value="3" @click="getValor('3')" title="Votar con 3 estrellas">&#9733;</a>
                <a href="#" data-value="4" @click="getValor('4')" title="Votar con 4 estrellas">&#9733;</a>
                <a href="#" data-value="5" @click="getValor('5')" title="Votar con 5 estrellas">&#9733;</a>
            </div>
        </div>
        <div v-else>
            <h6>Gracias por calificarnos!</h6>
        </div>
        <a id="bottom"></a>
    </section>
</main>
